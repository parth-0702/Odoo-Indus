...
export type StockLedgerDTO = {
  _id: string;
  productId: string;
  warehouseId: string;
  operationType: 'receipt' | 'delivery' | 'transfer_in' | 'transfer_out' | 'adjustment' | 'initial' | string;
...
export type BulkStockBalanceDTO = { productId: string; warehouseId: string; quantity: number };
...
export const api = {
  ...
  operations: {
    ...
    cancel: (id: string) => request<OperationDTO>(`/api/operations/${id}/cancel`, { method: 'POST' }),
  },
  stock: {
    ...
    balanceBulk: (warehouseId: string, productIds?: string[]) => {
      const qs = new URLSearchParams();
      qs.set('warehouseId', warehouseId);
      if (productIds?.length) qs.set('productIds', productIds.join(','));
      return request<BulkStockBalanceDTO[]>(`/api/stock/balance/bulk?${qs.toString()}`);
    },
    ledger: (params?: { productId?: string; warehouseId?: string; type?: string; limit?: number }) => {
      ...
      return request<StockLedgerDTO[]>(`/api/stock/ledger${suffix ? `?${suffix}` : ''}`);
    },
  },
};
