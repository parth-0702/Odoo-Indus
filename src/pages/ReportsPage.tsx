...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setProducts(p); setWarehouses(w); setCategories(c);
    };
    void load();
  }, [refreshStock]);
  ...
  const warehouseData = warehouses.map(w => {
    const stock = products.reduce((s, p) => s + getStock(p._id, w._id), 0);
  ...
  const exportCSV = () => {
    const rows = [['Product', 'SKU', 'Category', 'Stock', 'Reorder Level', 'Status']];
    productStocks.forEach(p => rows.push([p.name, p.sku, p.catName, String(p.stock), String(p.reorderLevel ?? 0), p.status ?? 'active']));
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <PageHeader title="Reports" subtitle="Inventory analytics and summaries">
        <Button onClick={exportCSV} variant="outline" size="sm" className="border-border text-foreground gap-1.5">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <div className="card-elevated rounded-xl p-4">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              <Pie data={warehouseData} cx="50%" cy="50%" outerRadius={90} paddingAngle={3} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} labelLine={false}>
