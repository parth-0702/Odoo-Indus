...
export default function MoveHistoryPage() {
  ...
  const getProductName = (id: string) => products.find(p => p._id === id)?.name ?? `Product`;
  ...
  const filtered = useMemo(() => {
    return ledger.filter(e => {
      ...
      const entryDate = e.createdAt ? new Date(e.createdAt) : new Date(0);
  ...
  }, [ledger, search, filterWarehouse, filterOp, filterDateFrom, filterDateTo, products]);
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                <tr key={e._id ?? `${e.createdAt}-${idx}`} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
