...
export default function ProfilePage() {
  ...
  const saveProfile = async () => {
    if (!user) return;
    // TODO: wire to server endpoint (e.g. PUT /api/users/me)
    login({ ...user, fullName: profile.fullName, email: profile.email });
    setMsg({ type: 'success', text: 'Profile updated locally. Server profile update is not implemented yet.' });
    setTimeout(() => setMsg(null), 3000);
  };

  const savePassword = async () => {
    // TODO: wire to server endpoint (e.g. POST /api/auth/change-password)
    if (!user) return;
    ...
    setMsg({ type: 'success', text: 'Password change is not implemented on the server yet.' });
    setTimeout(() => setMsg(null), 3000);
  };
