...
import { z } from 'zod';
import { listCategories, listUnits, listWarehouses, createProduct, updateProduct, getProduct, createAndPostOperation } from '@/lib/repo';
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      if (isEdit && id) {
        ...
        // Optional initial stock: record as an adjustment operation (posted immediately)
              ...
              <Controller name="categoryId" control={control} render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  <SelectContent className="bg-popover border-border">
                    {categories.map(c => (
                      <SelectItem key={c._id} value={String(c._id)} className="text-popover-foreground">
                        {c.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <div>
              ...
              <Controller name="unitId" control={control} render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  <SelectContent className="bg-popover border-border">
                    {units.map(u => (
                      <SelectItem key={u._id} value={String(u._id)} className="text-popover-foreground">
                        {u.name} ({u.symbol})
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <div>
              ...
              <Controller name="warehouseId" control={control} render={({ field }) => (
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  <SelectContent className="bg-popover border-border">
                    {warehouses.map(w => (
                      <SelectItem key={w._id} value={String(w._id)} className="text-popover-foreground">
                        {w.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
